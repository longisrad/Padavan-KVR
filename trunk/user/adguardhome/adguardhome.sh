...
# Bảo vệ NAND: sau khi AGH đã tự mở rộng $adg_file (có statistics:/querylog: block),
# chuyển dir_path của 2 mục này sang RAM để tránh ghi liên tục vào flash.
mkdir -p /tmp/adguard-log
if grep -q '^statistics:' "$adg_file"; then
  sed -i '/^statistics:/,/^[a-z]/{s|  dir_path: ""|  dir_path: "/tmp/adguard-log"|}' "$adg_file"
fi
if grep -q '^querylog:' "$adg_file"; then
  sed -i '/^querylog:/,/^[a-z]/{s|  dir_path: ""|  dir_path: "/tmp/adguard-log"|}' "$adg_file"
fi
}
  ...
  sleep 4
  	if [ ! -z "`pidof AdGuardHome`" ] ; then
		...
		logger -t "【AdGuardHome】" "Running successfully!"
  		...
  		# Chờ AGH thực sự lắng nghe port 5335 rồi mới chuyển DNS sang nó,
  		# tránh khoảng trống DNS lúc AGH chưa kịp bind (nguyên nhân gây gián đoạn lúc start/restart)
  		retry=0
  		while [ $retry -lt 10 ]; do
  			netstat -uln 2>/dev/null | grep -q ":5335 " && break
  			sleep 1
  			retry=$((retry + 1))
  		done
  		[ $retry -eq 10 ] && logger -t "【AdGuardHome】" "WARNING: AGH did not bind port 5335 after 10s, switching DNS anyway"
  		change_dns
  		set_iptable
  		adg_restart o
