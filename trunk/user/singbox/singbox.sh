...
build_dns_block() {
    ...
    # Rule nay PHAI khac nhau theo tung dns_mode vi server tag ton tai khac
    # nhau o moi nhanh (vd "dns-remote" chi ton tai o nhanh FakeIP).
    case "$dns_mode" in
    0)
        cat <<-EOF
  "dns": {
    ...
    "rules": [
      { "inbound": "dns-in-agh", "server": "dns-direct" }
    ],
    "final": "dns-direct",
...
EOF
  ...
  "dns": {
    ...
    "rules": [
      { "inbound": "dns-in-agh", "server": "dns-doh" }
    ],
    "final": "dns-doh",
    "independent_cache": true
  },
EOF
  ...
  "dns": {
    ...
    "rules": [
      { "inbound": "dns-in-agh", "server": "dns-remote" },
...
'
    ...
    *) default_resolver_tag="dns-direct" ;;
    ...
    if [ "$rt_mode" = "0" ]; then
      ...
      { "inbound": "dns-in-agh", "action": "sniff" },
'
    else
