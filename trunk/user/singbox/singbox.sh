...
fetch_all_groups() {
    ...
    while [ "$i" -lt "$count" ]; do
        ...
        # Bỏ qua các Sub đang bị TẮT công tắc ON/OFF
...
generate_config() {
    ...
    # Mặc định TẮT các tính năng nếu NVRAM chưa thiết lập (khớp defaults.c)
...
# ĐIỀU HƯỚNG IPTABLES TPROXY TỐI ƯU BẰNG IPSET $O(1)$ HASH LOOKUP
apply_iptables_mode() {
    sb_mode="$(nv singbox_mode)"
    if [ "$sb_mode" = "1" ]; then
        log "Applying IPTables TPROXY rules (IPSet $O(1)$ Hash Optimized)..."
        ...
        # Dọn sạch quy tắc cũ trong vòng lặp tránh đọng rác
        while ip rule del fwmark 1 table 100 2>/dev/null; do :; done
        ip rule add fwmark 1 table 100 2>/dev/null
        ip route add local default dev lo table 100 2>/dev/null

        # 1. TẠO TẬP HỢP IPSET BYPASS (TỐC ĐỘ HASH BẢNG $O(1)$)
        ...
        # 2. TẠO CHUỖI MANGLE TPROXY
        iptables -t mangle -N SINGBOX 2>/dev/null
        iptables -t mangle -F SINGBOX 2>/dev/null
        
        # CHỈ CẦN 1 DÒNG DUY NHẤT ĐỂ CHECK TOÀN BỘ DẢI IP BYPASS!
...
clean_iptables() {
    ...
    # Xóa tập hợp IPSet khi ngắt TProxy
