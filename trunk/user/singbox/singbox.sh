#!/bin/sh
# sing-box lifecycle manager for Newifi 3 D2 (MT7621AT - TProxy & Tailscale & AdGuard Home Edition)
...
download_jq() {
    ...
    if ! "$tmp_jq" --version >/dev/null 2>&1; then
        log "Binary jq tải về không chạy được"
        rm -f "$tmp_jq"
        return 1
    fi

    log "Đã tải jq vào RAM: $JQ_STORAGE_PATH ($size bytes)"
    return 0
}
...
fetch_all_groups() {
    if [ -s "/etc/storage/singbox_sub.json" ]; then
        sub_list_json="$(cat /etc/storage/singbox_sub.json)"
    else
    ...
    if ! have_jq; then
        log "CANH BAO: thiếu 'jq' -> không thể tự động gộp nhóm từ Subscription."
        return 1;
    fi
...
generate_config() {
    ...
    # MỞ TPROXY 7893 + CỔNG DNS 5353 CHO ADGUARD HOME KẾT NỐI
    if [ "$mode" = "0" ]; then
...
# BỔ SUNG LỚP BẢO VỆ CHO TAILSCALE (BYPASS 100.64.0.0/10)
apply_iptables_mode() {
    ...
    if [ "$sb_mode" = "1" ]; then
        ...
        # Loại trừ các dải IP nội bộ & DẢI IP CỦA TAILSCALE (100.64.0.0/10)
        iptables -t mangle -A SINGBOX -d 0.0.0.0/8 -j RETURN
        iptables -t mangle -A SINGBOX -d 10.0.0.0/8 -j RETURN
        iptables -t mangle -A SINGBOX -d 100.64.0.0/10 -j RETURN   # TAILSCALE BYPASS
        ...
        log "Kích hoạt TProxy toàn mạng LAN + Bypass Tailscale thành công!"
    else
        clean_iptables
    fi
}
