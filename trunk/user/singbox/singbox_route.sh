...
# Vị trí: /etc/storage/singbox_route.sh hoặc /etc_ro/singbox/singbox_route.sh
# ==============================================================================
...
start_routing() {
    ...
    # 3. Tạo Bảng định tuyến Policy Routing Table 100 với ưu tiên cao nhất (pref 100) cho UDP TProxy
    ...
    # 5. BẺ LÁI TCP SANG NAT REDIRECT (CỔNG 7892) - Ổn định tuyệt đối trên Padavan
    ...
    # 6. BẺ LÁI UDP SANG MANGLE TPROXY (CỔNG 7893) - Dành cho Game online / QUIC
    ...
    # 7. Mở khóa Kernel cho TProxy (ip_nonlocal_bind & rp_filter trên toàn bộ card)
    echo 1 > /proc/sys/net/ipv4/ip_nonlocal_bind 2>/dev/null
    for f in /proc/sys/net/ipv4/conf/*/rp_filter; do
        echo 0 > "$f" 2>/dev/null
    done

    # 8. Chuyển hướng DNS mạng LAN (Port 53 -> Port 5353) nếu bật tuỳ chọn
    ...
    # 9. Gỡ 127.0.0.1 và tự động thêm DNS dự phòng nếu file rỗng
...
stop_routing() {
    ...
    # 1. Gỡ chuỗi TCP Redirect (NAT)
    while iptables -t nat -D PREROUTING -i $LAN_IFACE -p tcp -j SINGBOX_TCP 2>/dev/null; do :; done
    iptables -t nat -F SINGBOX_TCP 2>/dev/null
    iptables -t nat -X SINGBOX_TCP 2>/dev/null

    # 2. Gỡ chuỗi UDP TProxy (Mangle)
    while iptables -t mangle -D PREROUTING -i $LAN_IFACE -p udp -j SINGBOX_UDP 2>/dev/null; do :; done
    iptables -t mangle -F SINGBOX_UDP 2>/dev/null
    iptables -t mangle -X SINGBOX_UDP 2>/dev/null
    ...
    # 4. Xóa Bảng định tuyến Table 100 & IPSet
    while ip rule del fwmark 1 table 100 2>/dev/null; do :; done
    while ip rule del pref 100 2>/dev/null; do :; done
...
case "$1" in
    ...
    restart)
        ...
        ;;
    status)
        if iptables -t nat -L PREROUTING -n -v | grep -q SINGBOX_TCP; then
            echo "Trạng thái định tuyến: ĐANG BẬT (ACTIVE - HYBRID)"
            exit 0
        else
