...
' ;;
    ...
    # Khi Hijack DNS bat (dns_redirect=1) VA dang o TProxy mode (dns-in chi
    # ton tai o mode nay - them rule tham chieu inbound khong ton tai o
    # Mixed mode se lam sing-box tu choi parse config): dieu huong TOAN BO
    # query den tu client bi hijack (inbound "dns-in") sang AGH truoc tien,
    # bo qua fakeip/dns-remote/dns-direct hoan toan cho luong nay. AGH tu
    # quyet dinh chan (tra 0.0.0.0) hay cho qua (tra IP that tu upstream
    # cua chinh AGH). TPROXY van chan dung dua tren IP THAT ma AGH tra ve
    # (khong can fakeip), nen geoip-vn/adblock van hoat dong binh thuong.
    dns_agh_server=""
    dns_agh_rule_bare=""
...
'
    ...
    # Ghep 2 rule "bare" (khong dau phay) cho case 0/2 - CHI co the co
    # dns_agh_rule_bare va/hoac agh_rule_bare, KHONG co rule nao khac theo
    # sau trong 2 nhanh nay. Dung join thu cong bang dau phay de tranh loi
    # "dau phay treo" khi 1 trong 2 rong.
    rules_02=""
...
EOF
  ...
  "dns": {
    "servers": [
${dns_agh_server}      { "tag": "dns-remote", "type": "tls", "server": "8.8.8.8", "detour": "${dns_final_detour}" },
