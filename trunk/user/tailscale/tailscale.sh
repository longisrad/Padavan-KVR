...
# Tích hợp tự động dọn sạch Cache, Routing Table 52 & Chống nghẽn mạng
# ==============================================================================
...
kill_ts() {
	...
	# 1. QUÉT SẠCH TÀN DƯ ROUTING TABLE 52
	while ip rule del table 52 2>/dev/null; do :; done
	ip route flush table 52 2>/dev/null

	# 2. DỌN SẠCH MAGICDNS NẾU BỊ DÍNH TRONG RESOLV.CONF
	if [ -f /etc/resolv.conf ]; then
		sed -i '/^nameserver 100\./d' /etc/resolv.conf
	fi

	# 3. TỰ ĐỘNG XẢ SẠCH TOÀN BỘ CACHE KERNEL & SING-BOX (TRẢ LẠI MẠNG MƯỢT MÀ)
	ip tcp_metrics flush 2>/dev/null
...
stop_ts() {
	...
	log "Tailscale đã dừng sạch sẽ và giải phóng toàn bộ Cache!"
}
